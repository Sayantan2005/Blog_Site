...
function PopularAuthors() {
    ...
    const getAllUsers = async () => {
        try {
            const res = await axios.get(`https://blog-site-2-pzsc.onrender.com/api/v1/user/all-users`)
            if (res.data.success) {
    ...
    return (
        <div>
            <div className='max-w-7xl mx-auto'>
                ...
                <div className='flex items-center justify-around my-10 px-4 md:px-0 pb-10'>
                    {
                        popularUser.slice(0, 3)?.map((user, index) => {
                            return <div key={index} className='flex flex-col gap-2 items-center'>
                                <img src={user.photoURL || userLogo}  className="rounded-full h-16 w-16 md:w-32 md:h-32" alt="" />
